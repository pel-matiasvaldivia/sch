...
export function UserForm({ mode, user }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 max-w-lg">
      ...
      {/* Aviso cuando se selecciona rol médico — redirigir a configuración */}
      {isMedico && !isEdit && (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-amber-900">
                Los médicos deben crearse desde Configuración
              </p>
              <p className="text-sm text-amber-700 mt-1">
                El alta de médicos se gestiona desde la sección de Configuración del sistema, donde se pueden completar todos los datos profesionales.
              </p>
              <button
                type="button"
      ...
      {/* Aviso cuando se selecciona rol paciente — redirigir a pacientes */}
      {isPaciente && !isEdit && (
        <div className="border border-amber-200 bg-amber-50 rounded-xl p-4">
          <div className="flex items-start gap-3">
            <svg className="w-5 h-5 text-amber-600 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
              <p className="text-sm font-medium text-amber-900">
                Los pacientes deben crearse desde la sección de Pacientes
              </p>
              <p className="text-sm text-amber-700 mt-1">
                Al registrar un paciente, se crea automáticamente su usuario de acceso. Usá el formulario de pacientes para ingresar todos los datos clínicos.
              </p>
              <button
                type="button"
